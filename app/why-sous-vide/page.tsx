...
export default function WhySousVide() {
  return (
    <main className="bg-green-deep text-cream">
      <Nav dark />
      ...
      <header className="mx-auto max-w-7xl px-5 pb-16 pt-36 md:px-8 md:pt-44">
        <Reveal>
          ...
          <h1 className="shout mt-5 max-w-4xl text-7xl md:text-[7.5rem]">
            Sous vide is the <span className="text-lime-brand">whole trick.</span>
          </h1>
      ...
      <section className="py-24 md:py-32">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <Reveal>
            <p className="section-label text-lime-brand !opacity-90">The process</p>
            <h2 className="shout mt-3 text-6xl md:text-8xl">Five steps. Zero shortcuts.</h2>
          </Reveal>
          <div className="mt-14 space-y-4">
            {STEPS.map((s, i) => (
              <Reveal key={s.num} delay={i * 0.05}>
                <div className="grid items-start gap-4 rounded-3xl bg-white/5 p-7 md:grid-cols-[100px_280px_1fr] md:gap-8">
                  <p className="stat-num text-6xl text-lime-brand">{s.num}</p>
                  <h3 className="shout-upright pt-1 text-4xl">{s.title}</h3>
                  <p className="pt-1.5 leading-relaxed text-cream/65">{s.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      {/* Why it wins */}
      <section className="border-t border-white/10 bg-black/20 py-24">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <Reveal>
            <h2 className="shout max-w-3xl text-6xl md:text-8xl">
              What you get <span className="text-lime-brand">out of it.</span>
            </h2>
          </Reveal>
          <div className="mt-14 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              ['27g', 'Protein, intact', 'No protein loss to boiling water, no fat needed for the pan. The macros on the pouch are the macros in the meat.'],
              ...
              <Reveal key={title} delay={i * 0.07}>
                <div className="h-full rounded-3xl border border-white/15 p-7">
                  <p className="stat-num text-7xl text-lime-brand">{stat}</p>
                  <h3 className="shout-upright mt-4 text-2xl">{title}</h3>
                  <p className="mt-2.5 text-sm leading-relaxed text-cream/60">{body}</p>
                </div>
              </Reveal>
            ))}
          </div>
